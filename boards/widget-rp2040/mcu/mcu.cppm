...
export module platform.widget_rp2040.mcu;
...
// state; the bank selects which peripheral owns a pin; the pad controls the
// input buffer and the pulls; the reset block must release both banks first.
...
void release_banks() {
    ...
    constexpr unsigned long wanted = (1UL << 5) | (1UL << 8);
...
class WidgetRp2040Platform : public mm::mcu::Platform {
public:
    [[nodiscard]] mm::mcu::Board board() const override {
        return {"widget-rp2040", gpios,
                mm::mcu::Led{"LED", widget_rp2040_status_led_pin(), true}};
    }

    [[nodiscard]] mm::mcu::Status gpio_configure(
        unsigned int pin, mm::mcu::Direction direction,
        ...
        // OE_SET / OE_CLR
        reg(sio_base + (direction == mm::mcu::Direction::Out ? 0x24 : 0x28)) =
            mask;
    ...
    [[nodiscard]] mm::mcu::Status gpio_write(unsigned int pin,
                                             bool high) override {
        if (pin >= pin_count) return mm::mcu::Status::BadArgument;
        reg(sio_base + (high ? 0x14 : 0x18)) = 1UL << pin;  // OUT_SET/CLR
        return mm::mcu::Status::Ok;
    }
...
WidgetRp2040Platform widget_rp2040_platform;

struct Register {
    Register() { mm::mcu::set_platform(widget_rp2040_platform); }
};
