...
void eleven_operations_are_present() {
    bool ok = false;
    auto loaded = mm::model::Loaded::load(".", ok);
    const auto operations = loaded.operations();
    mm::test::expect(operations.size() == 11,
                     "expected exactly the eleven documented operations");
}
...
void test_invokes_the_test_runner_eleven_times() {
    bool ok = false;
    auto loaded = mm::model::Loaded::load(".", ok);
    ...
    mm::test::expect(count == 11,
                     "expected test.sh to invoke the test runner exactly eleven times");
}
...
void recommended_sequence_matches_the_documented_order() {
    ...
    mm::test::expect(ordered.size() == 11, "expected the recommended sequence to cover all eleven");
    if (ordered.size() != 11) return;

    const std::string_view expected[11] = {
        "clean", "bootstrap", "configure", "build", "test", "document", "check", "model", "run",
        "flash", "debug",
    };
...
const mm::test::case_ cases[] = {
    { "eleven operations are present",                &eleven_operations_are_present },
    ...
    { "test invokes the test runner eleven times",     &test_invokes_the_test_runner_eleven_times },
