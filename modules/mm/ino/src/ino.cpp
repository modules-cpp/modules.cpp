...
std::string sketch_header() {
    ...
    out += "//\n";
    out += "// A vendored sketch library includes this header by name,\n";
...
bool check_application(const std::filesystem::path& app_dir,
    ...
    // same terms as main.cpp, so it is verified on the same terms too.
    const auto* libraries = lookup(doc, "sketch-library");
