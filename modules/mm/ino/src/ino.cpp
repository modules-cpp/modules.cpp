...
std::string sketch_header() {
    ...
    out += "// main.cpp, which includes it, and the build puts this directory\n";
    out += "// on the include path. The names a library includes by reach it\n";
    out += "// through the forwarding headers written beside it. Names below\n";
    out += "// come from mm.sketch, <cstdint>, and the compatibility\n";
    out += "// environment.\n";
    out += "#pragma once\n";
    out += "\n";
    out += "#include <cstddef>\n";
    ...
    out += "// library reaches through this header rather than by naming\n";
    ...
    // The mathematical constants a sketch toolchain defines as macros. A
    ...
    // The cooperative hook a sketch calls inside its own waiting loop. A
    // sketch toolchain spells it yield; this project spells the same
    ...
    // AVR's binary.h, which a sketch reaches through this header. Every bit
...
std::span<const std::string_view> sketch_alias_headers() {
    // The spellings a vendored source writes. Arduino.h is first because it
    // is the one every such source writes, and it is here rather than in the
    // project's own vocabulary for exactly that reason: no rename reaches
    // inside a checkout, so the name survives as a forwarder and nothing
    // else. The others each name a class mm.sketch already exports: Print
    // and Printable from its own section, Wire from the I2C section, SPI
...
std::string sketch_alias_header(std::string_view name) {
    ...
    out += std::string(name);
    out += " because its own\n";
    out += "// toolchain ships one. Everything it declares is in ";
    out += std::string(sketch_header_name());
    out += "\n";
...
bool check_application(const std::filesystem::path& app_dir,
    ...
    const std::string canonical(sketch_header_name());
    const std::filesystem::path header = app_dir / canonical;
    std::ifstream in_header(header);
    if (!in_header) {
        error = "missing generated " + canonical + " in " + app_dir.string();
        return false;
    }
