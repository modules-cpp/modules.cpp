...
std::vector<std::unique_ptr<models::Operation>> build_operations(
    ...
    // (27 suites); branch 1 is Darwin, which skips tests/mm/linux/ because
    // Linux DRM headers are unavailable on macOS (26 suites). A suite
    // added to test.sh must update these two counts with it.
    {
        ...
        for (std::size_t darwin = 0; darwin < 2; ++darwin) {
            ...
            const std::size_t suites = darwin == 0 ? 27 : 26;
