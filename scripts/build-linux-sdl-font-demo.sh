...
verify_fonts() {
    ...
    for table in kMono12 kMono16; do
        if ! $nm_command -C "$image" | grep -q "mm::fonts::${table}Data"; then
            echo "$test_name: no mm::fonts::$table table in $image" >&2
            exit 1
        fi
    done
}
...
echo "then every glyph of the 12px table, wrapped to the window."
