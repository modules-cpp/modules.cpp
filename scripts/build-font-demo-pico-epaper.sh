#!/bin/sh
# Build apps/font-demo for a Raspberry Pi Pico with a Waveshare Pico-ePaper-2.66
...
# font-demo names exactly those two interfaces, so the image must carry one
...
echo "  scripts/build-font-demo-pico-epaper.sh --panel $panel --flash"
echo "Four full refreshes, four seconds apart: four centred lines, two at 16px"
echo "and two at 12px, the last one Polish, then every glyph of the 12px table,"
echo "black on white, upright first and then turned a quarter clockwise each"
