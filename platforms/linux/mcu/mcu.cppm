...
const platform::linux::Map* map(Status& status) {
    ...
    if (result.status != platform::linux::MapStatus::Ok || result.map == nullptr) {
...
public:
    [[nodiscard]] mm::mcu::Board board() const override {
        ...
        if (!board_ready_) {
            ...
            for (std::size_t i = 0; i < configured->gpios.size(); ++i)
                gpio_inventory_.push_back({static_cast<unsigned int>(i),
                                           configured->gpios[i].name});
            board_ready_ = true;
        }
        std::optional<mm::mcu::Led> led;
        if (configured->led_gpio)
            led = mm::mcu::Led{configured->led_name.value_or("LED"),
                               *configured->led_gpio, configured->led_active_high};
        return {configured->board_name, gpio_inventory_, led};
    }

    [[nodiscard]] Status gpio_configure(unsigned int pin, mm::mcu::Direction direction,
        ...
        request.num_lines = 1;
        std::strncpy(request.consumer, "modules.cpp", sizeof(request.consumer) - 1);
        request.config.flags = direction == mm::mcu::Direction::Out
                                   ? GPIO_V2_LINE_FLAG_OUTPUT : GPIO_V2_LINE_FLAG_INPUT;
        if (pull == mm::mcu::Pull::Up) request.config.flags |= GPIO_V2_LINE_FLAG_BIAS_PULL_UP;
        if (pull == mm::mcu::Pull::Down) request.config.flags |= GPIO_V2_LINE_FLAG_BIAS_PULL_DOWN;
    ...
    [[nodiscard]] Status spi_configure(const mm::mcu::SpiConfiguration& value) override {
        Status status; const auto* configured=map(status); if(!configured)return status;
        ...
        std::uint8_t lsb=value.bit_order==mm::mcu::BitOrder::LeastSignificantFirst;
        ...
        if(::ioctl(fd.get(),SPI_IOC_WR_MODE32,&mode)<0||
           ...
           (!entry.speed_fixed&&::ioctl(fd.get(),SPI_IOC_WR_MAX_SPEED_HZ,&speed)<0))
            return error_status(errno,true,true);
    ...
    [[nodiscard]] Status i2c_configure(const mm::mcu::I2cConfiguration& value) override {
        Status status;const auto* configured=map(status);if(!configured)return status;
        if(value.instance>=configured->i2cs.size())return Status::Unsupported;
        const auto& entry=configured->i2cs[value.instance];
        if(value.data_gpio!=entry.data_gpio||value.clock_gpio!=entry.clock_gpio)return Status::BadArgument;
        if(!entry.baud_fixed)return Status::Unsupported;
        if(value.baud!=entry.baud)return Status::BadArgument;
        if(i2c_configuration_.size()<=value.instance)i2c_configuration_.resize(value.instance+1);
    ...
    [[nodiscard]] Status i2c_write_read(unsigned int instance,unsigned int address,
                                         std::span<const std::byte> command,
                                         std::span<std::byte> data) override {
        return i2c_transaction(instance,address,command,data);
    }

    [[nodiscard]] Status uart_write(unsigned int instance,const char* text) override {
        ...
        const auto baud=baud_rate(entry.baud);if(baud==0)return Status::BadArgument;
        Descriptor fd(::open(entry.path.c_str(),O_WRONLY|O_NOCTTY|O_NONBLOCK|O_CLOEXEC));
        if(fd.get()<0)return error_status(errno);
        termios tty{};if(::tcgetattr(fd.get(),&tty)<0)return error_status(errno);
        ::cfmakeraw(&tty);::cfsetispeed(&tty,baud);::cfsetospeed(&tty,baud);
        tty.c_cflag=(tty.c_cflag&~CSIZE)|(entry.data_bits==5?CS5:entry.data_bits==6?CS6:entry.data_bits==7?CS7:CS8);
        if(entry.parity==0)tty.c_cflag&=~PARENB;else{tty.c_cflag|=PARENB;if(entry.parity==2)tty.c_cflag|=PARODD;else tty.c_cflag&=~PARODD;}
        if(entry.stop_bits==2)tty.c_cflag|=CSTOPB;else tty.c_cflag&=~CSTOPB;
        if(::tcsetattr(fd.get(),TCSANOW,&tty)<0)return error_status(errno,true,true);
        const auto size=std::strlen(text);std::size_t done=0;
        const auto deadline=std::chrono::steady_clock::now()+std::chrono::milliseconds(entry.write_deadline_ms);
        while(done<size){const auto count=::write(fd.get(),text+done,size-done);if(count>0){done+=count;continue;}if(count<0&&errno==EINTR)continue;if(count<0&&(errno==EAGAIN||errno==EWOULDBLOCK)){const auto left=std::chrono::duration_cast<std::chrono::milliseconds>(deadline-std::chrono::steady_clock::now()).count();if(left<=0)return Status::Timeout;pollfd p{fd.get(),POLLOUT,0};int ready;do ready=::poll(&p,1,static_cast<int>(left));while(ready<0&&errno==EINTR);if(ready==0)return Status::Timeout;if(ready<0)return error_status(errno);continue;}return error_status(errno);}
        return Status::Ok;
    }
    ...
    [[nodiscard]] Status delay_ms(unsigned long milliseconds) override {
        ...
        if(seconds>static_cast<unsigned long>(std::numeric_limits<time_t>::max()))return Status::BadArgument;
        timespec request{static_cast<time_t>(seconds),static_cast<long>((milliseconds%1000)*1'000'000UL)};
        while(true){timespec remaining{};const int result=::clock_nanosleep(CLOCK_MONOTONIC,0,&request,&remaining);if(result==0)return Status::Ok;if(result==EINTR){request=remaining;continue;}return error_status(result,false);}
    }

    [[nodiscard]] Status ticks_ms(unsigned long& ticks) override {
        timespec value{};if(::clock_gettime(CLOCK_MONOTONIC,&value)<0)return error_status(errno,false);
        ticks=static_cast<unsigned long>(value.tv_sec)*1000UL+static_cast<unsigned long>(value.tv_nsec/1'000'000L);
        return Status::Ok;
    }
...
private:
    ...
    Status spi_exchange(unsigned int instance,std::span<const std::byte> transmit,
    ...
    Status i2c_transaction(unsigned int instance,unsigned int address,
        ...
        if(address>0x7f||(!out.size()&&!in.size()))return Status::BadArgument;
        Status status;const auto* configured=map(status);if(!configured)return status;
        if(instance>=configured->i2cs.size())return Status::Unsupported;
        if(instance>=i2c_configuration_.size()||!i2c_configuration_[instance])return Status::BadArgument;
        const auto path="/dev/i2c-"+std::to_string(configured->i2cs[instance].adapter);Descriptor fd(::open(path.c_str(),O_RDWR|O_CLOEXEC));if(fd.get()<0)return error_status(errno);
        i2c_msg messages[2]{};unsigned int count=0;if(!out.empty()){messages[count].addr=address;messages[count].len=out.size();messages[count].buf=reinterpret_cast<__u8*>(const_cast<std::byte*>(out.data()));++count;}if(!in.empty()){messages[count].addr=address;messages[count].flags=I2C_M_RD;messages[count].len=in.size();messages[count].buf=reinterpret_cast<__u8*>(in.data());++count;}i2c_rdwr_ioctl_data data{messages,count};if(::ioctl(fd.get(),I2C_RDWR,&data)<0)return error_status(errno);return Status::Ok;
    }
